 ...
 * (3) Save like data
 */
add_action( 'wp_ajax_nopriv_jm-post-like', 'ivan_post_like' );
add_action( 'wp_ajax_jm-post-like', 'ivan_post_like' );
function ivan_post_like() {
	$nonce = $_POST['nonce'];
	...
	if ( isset( $_POST['jm_post_like'] ) ) {
		...
		if ( is_user_logged_in() ) { // user is logged in
			...
			if ( !ivan_AlreadyLiked( $post_id ) ) { // like the post
 ...
 */
function ivan_AlreadyLiked( $post_id ) { // test if user liked before
	if ( is_user_logged_in() ) { // user is logged in
		$user_id = get_current_user_id(); // current user
		$meta_USERS = get_post_meta( $post_id, "_user_liked" ); // user ids from post meta
 ...
 * (5) Front end button
 */
function ivan_getPostLikeLink( $post_id ) {
	$like_count = get_post_meta( $post_id, "_post_like_count", true ); // get post likes
	$count = ( empty( $like_count ) || $like_count == "0" ) ? '' : esc_attr( $like_count );
	if ( ivan_AlreadyLiked( $post_id ) ) {
		$class = esc_attr( ' liked' );
		$title = esc_attr( esc_html__('Unlike', 'bomby') );
		$heart = '<i class="fa fa-heart"></i>';
	} else {
		$class = esc_attr( '' );
		$title = esc_attr( esc_html__('Like', 'bomby') );
 ...
 */
add_action( 'show_user_profile', 'ivan_show_user_likes' );
add_action( 'edit_user_profile', 'ivan_show_user_likes' );
function ivan_show_user_likes( $user ) { ?>        
    <table class="form-table">
        <tr>
			<th><label for="user_likes"><?php esc_html_e( 'You Like:', 'bomby' ); ?></label></th>
			<td>
            <?php
			...
			if ( $count > 0 ) {
				...
				echo wp_kses_data($like_list);
			} else {
				echo "<p>" . esc_html_e( 'You don\'t like anything yet.', 'bomby' ) . "</p>\n";
			} ?>
            </td>
		</tr>
    </table>
 ...
 */
function ivan_frontEndUserLikes() {
	if ( is_user_logged_in() ) { // user is logged in
		$like_list = '';
		$user_id = get_current_user_id(); // current user
		...
		if ( $count > 0 ) {
			...
			$like_list .= "<aside>\n";
			$like_list .= "<h3>" . esc_html__( 'You Like:', 'bomby' ) . "</h3>\n";
			$like_list .= "<ul>\n";
 ...
 * Markup assumes sidebar/widget usage
 */
function ivan_most_popular_today() {
	global $post;
	$today = date('j');
  	$year = date('Y');
	$args = array(
	...
	if ( $pop_posts->have_posts() ) {
		echo "<aside>\n";
		echo "<h3>" . esc_html_e( 'Today\'s Most Popular Posts', 'bomby' ) . "</h3>\n";
		echo "<ul>\n";
 ...
 * Markup assumes sidebar/widget usage
 */
function ivan_most_popular_month() {
	global $post;
	$month = date('m');
  	$year = date('Y');
	$args = array(
	...
	if ( $pop_posts->have_posts() ) {
		echo "<aside>\n";
		echo "<h3>" . esc_html_e( 'This Month\'s Most Popular Posts', 'bomby' ) . "</h3>\n";
		echo "<ul>\n";
		while ( $pop_posts->have_posts() ) {
			$pop_posts->the_post();
			echo "<li><a href='" . esc_url(get_permalink($post->ID)) . "'>" . get_the_title() . "</a></li>\n";
 ...
 * Markup assumes sidebar/widget usage
 */
function ivan_most_popular_week() {
	global $post;
	$week = date('W');
  	$year = date('Y');
	$args = array(
	...
	if ( $pop_posts->have_posts() ) {
		echo "<aside>\n";
		echo "<h3>" . esc_html_e( 'This Week\'s Most Popular Posts', 'bomby' ) . "</h3>\n";
		echo "<ul>\n";
		while ( $pop_posts->have_posts() ) {
			$pop_posts->the_post();
			echo "<li><a href='" . esc_url(get_permalink($post->ID)) . "'>" . get_the_title() . "</a></li>\n";
 ...
 * Markup assumes sidebar/widget usage
 */
function ivan_most_popular() {
	global $post;
	echo "<aside>\n";
	echo "<h3>" . esc_html_e( 'Most Popular Posts', 'bomby' ) . "</h3>\n";
	echo "<ul>\n";
	$args = array(
		 'post_type' => array( 'post', 'enter-your-comma-separated-post-types-here' ),
		 'meta_key' => '_post_like_count',
